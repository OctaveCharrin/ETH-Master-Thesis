...
% Theorem environments (declared before cleveref so it picks up the names)
\theoremstyle{plain}
\newtheorem{proposition}{Proposition}[chapter]
%
% Fonts
\usepackage{newpxtext,newpxmath} % Palatino font, remove or change according to your taste
\usepackage[scaled=0.92]{helvet}  % Nimbus Sans for \sffamily (newpx's Heros/qhv is not installed here)
...
%
% Bibliography
\usepackage[numbers]{natbib} % numeric citations, matching the abbrv (numeric) bibliography style
%
...
%
% Draft helpers: visible open-question callouts and placeholder floats.
% Remove (or redefine \gap as \relax) once the draft is complete.
% \long so that a gap may span paragraphs; \textcolor is short and would fail on a blank line.
\long\def\gap#1{{\color{eth7}\textbf{[TODO:~}{\itshape#1}\textbf{]}}}
\newcommand{\placeholderfig}[1]{%
    \begin{center}
    \fcolorbox{eth7}{white}{\parbox[c][3cm][c]{0.9\linewidth}{\centering\itshape
    Placeholder --- #1}}
    \end{center}}
%
% ETH Colors
...
% Chapter 5 carries six full-width floats in one section; with the defaults they
% queue up and all land after the text that discusses them. Raise the per-page
% allowance and lower the text minimum so a float can be placed where it is cited.
