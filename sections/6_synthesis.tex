...
\paragraph{The two decisions are not separable.} Removing either agent costs measurable QoE, and the
two halves cost asymmetrically: an application agent over a naive split recovers as little as 35\% of
the joint system, and a learned split under a conventional bitrate controller as little as 84\%
...
both terms go negative and the ratio stops meaning anything. And the decomposition follows the training
body rather than the evaluation body: transferred zero-shot to the packet-level backend it dissolves ---
the application-only ablation recovers 80\% rather than 35\% and the network-only ablation costs nothing
measurable --- whereas policies \emph{trained} on that backend restore it, both halves costing 31\% and
20\% against a training-seed spread of 0.019 (\cref{sec:p2eval:ns3native}). What degrades under transfer
is the learned bitrate controller more than the learned scheduler.
...
\paragraph{A surrogate can be good enough to train in and useless to choose in --- and the price of
using it is measurable.} Six policies trained
...
the backend the claim is about, or report the distribution over training seeds and select nothing. The
grading failure is also bidirectional, which is what makes it a property of the surrogate rather than of
the policies: used as a grader on policies trained on the realistic body, the surrogate ranks the best
of them second behind a deployed baseline in nine of nine cells
(\cref{sec:p2eval:ns3native}). And once the same comparison is run in the training direction, the
surrogate's convenience acquires a price tag: policies trained on the realistic body beat transferred
ones by $0.228$ QoE on identical cells, at a sixth of the seed spread, for roughly fifteen times the
wall-clock. That is a defensible trade to make deliberately and an expensive one to make by default.
...
measured on the analytical mock, and \emph{transferred} zero-shot to the packet-level backend they hold
for three of six independently trained policies and not for the other three, with absolute QoE not
portable in any case (\cref{sec:p2eval:ns3}). Trained on that backend instead the headline result
returns --- first in nine of nine cells, $0.228$ QoE ahead of mock-trained transfer on identical cells
(\cref{sec:p2eval:ns3native}) --- but that arm is unconditioned and covers one scenario, so every
conditioning, intent-transfer, and mid-call result in the chapter remains mock-trained and inherits the
transfer caveat. The headline multipath result is also
...
tighter of the two, has the same shape of answer: it decides in \SI{0.536}{\milli\second} on average
on a 16-core CPU host with no GPU, about 1.6\% of the frame interval it must fit inside, and
...
\begin{itemize}
  ...
  intended stepping stone. Training there rather than transferring to it is measured and is worth
  $0.228$ QoE (\cref{sec:p2eval:ns3native}), which is also the best available estimate of what the next
  fidelity step would cost to skip.
