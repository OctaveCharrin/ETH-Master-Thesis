...
agent over a naive split recovers as little as 39\% of the joint system, and a learned split under a
conventional bitrate controller as little as 83\%; both hold on the packet-level backend as well as the
analytical one (\cref{sec:p2eval:ablation}). Rate and split are coupled through the same bottleneck, and
a controller that fixes either one is optimizing against a constraint it could have moved.
...
earn. Two practical implications follow. One should not benchmark a multipath scheduler on a
...
spread between training runs of the same method, so the benchmark reports its own seed draw. And the
payoff scales with the application's intent as well as with the network: the stricter the delay budget,
the less slack there is to absorb a poor allocation, so the more the allocation matters.
...
measured on the analytical mock and confirmed in \emph{ranking} but not in absolute value on the
packet-level backend (\cref{sec:p2eval:ns3}), on a single training seed; and all of its policies are
trained on the mock, so what is established is that a mock-trained policy transfers, not that the
system learns on the realistic body (\cref{sec:p2impl:gaps}).
...
it reaches the per-alternative comparison and provably cannot when it does not, and joint rate-and-split
control pays off in proportion to how unevenly path capacity is distributed and how strictly the
application prices delay -- but they do delimit how far the quantitative results should be pushed.
...
agent, whose per-frame budget is the tighter constraint. It requires no new experiment: every
\texttt{evaluation\_results.json} in \texttt{rl-mpquic/runs/} already records per-call decision time for
both agents, with warm-up calls discarded so the first-call overhead of the tensor library is excluded.
Report the transport agent's mean and 99th percentile against the \SI{33}{\milli\second} frame budget
and against the microsecond-scale cost of the heuristic schedulers, and state that it grows linearly in
the path count, since the shared per-path encoder is evaluated once per candidate.}
...
it, which is the gap learned control fills.
...
the multipath system -- to bound how gracefully performance decays there. The multipath half is the
cheaper of the two and is already set up: two held-out configurations
(\texttt{rl-mpquic/configs/dynamic\_ood\_\{harsh,mild\}.yaml}) place every hazard rate outside the
range any policy trained over, and grading existing checkpoints on them is evaluation-only. That run
also settles the domain-randomization question left open in \cref{sec:p2eval:ablation}: what was
measured there is randomization's cost on the nominal environment, and these configurations are the
test of its intended benefit. The two outcomes carry different claims -- ``randomization trades nominal
performance for robustness'' is a substantially different and more useful finding than ``randomization
is negative'' -- so this is worth running before either is asserted.}
