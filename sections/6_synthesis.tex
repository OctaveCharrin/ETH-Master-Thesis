...
conventional bitrate controller as little as 84\% (\cref{sec:p2eval:ablation}). Rate and split are
...
(\cref{sec:p2eval:ns3}). All of its policies are trained on the mock, so whether the system \emph{learns}
on the realistic body is untested (\cref{sec:p2impl:gaps}); and since mock score does not predict
packet-level rank, the mock's standing as a training surrogate is an open question rather than a
technicality. The parity of the intent-conditioned policy with the per-intent specialists is a failure to
detect a difference at the measured training-seed spread, not a demonstration of equivalence, and the
specialists it matches were themselves not measurably specialized
(\cref{sec:p2eval:transfer,sec:p2eval:conditioning}).
...
on a 16-core CPU host with no GPU, about 1.5\% of the frame interval it must fit inside
(\cref{fig:p2eval:decision}). Both controllers are therefore compute-bound nowhere near their
deadlines, and the cost grows linearly in the candidate count in each case, since the shared per-path
encoder is evaluated once per path. \gap{add the 99th percentile for the transport agent alongside the
mean once \cref{fig:p2eval:decision} is regenerated --- a real-time claim is a claim about the tail,
and the single-path figures above are quoted with both. The distribution is already recorded in every
\texttt{evaluation\_results.json}.}
...
continues the trend. The multipath conditioning generalizes along the same axis but with a weaker
warrant: sweeping the intent between two profiles moves bitrate and quality smoothly and monotonically,
by 17--24 times the span a non-learning control shows over the same rollouts, but training there sampled
the intent space continuously, so the interior of the segment is in-distribution rather than unseen
(\cref{sec:p2eval:conditioning}). Extrapolation past the endpoints fails asymmetrically in both systems,
