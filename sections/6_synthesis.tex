...
look conditioned while behaving identically. Feature-wise modulation avoids this by construction, because
it changes the \emph{relative} salience of path features and thus which path wins. This is a design
principle, not a SCION-specific trick: any comparison-based selector conditioned on an objective must
...
profiles, so that a novel weight vector yields sensible behavior without retraining. This is currently
untested -- \cref{sec:p1eval:intent} evaluates only trained profiles, and the experiment that would
establish it is the subject of \cref{sec:p1eval:zeroshot}. \textbf{Across topologies and path counts:} the
permutation-equivariant design gives structural generalization, but only empirical results can confirm it
holds off the training distribution, and the single-path study evaluates one topology on pairs drawn from
its own training pool (\cref{sec:p1eval:setup}). \textbf{Across conditions:} the dynamic scenario of
\cref{ch:multipath}
