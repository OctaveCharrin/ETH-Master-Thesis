...
policy representations and feature modulation techniques for conditioning decisions on application intent.
...
\emph{How} the intent vector conditions the network architecture is critical. The mechanism
...
transform an intermediate feature representation $h$ as $\mathrm{FiLM}(h) = (1 + \gamma) \odot h + \beta$. 
...
bulk transfers -- directly altering which candidate path scores highest. This matters most for
comparison-based selection, where the action is an $\arg\max$ over per-alternative scores: a conditioning
signal that enters only terms shared by every alternative shifts all scores together and cancels under
the comparison, so the policy can consume the intent without ever acting on it. \Cref{sec:p1design:concat}
makes this precise and \cref{sec:p1eval:ablation} measures it.