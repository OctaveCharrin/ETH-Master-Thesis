...
\gap{three of the six properties below are currently asserted rather than measured for the single-path
system. \emph{Zero-Shot Generalization} is never tested: every intent reported in \cref{ch:single_path}
is one of the reward profiles the agent trained on, and no interpolated or unseen weight vector is ever
evaluated. \emph{Dynamic-Action Handling} is never tested: results are not broken down by candidate-path
count $N$. \emph{Structural \& Topological Generalization} is never tested: only one topology exists, and
the evaluation pairs are drawn from the same pool used for training. Either add the corresponding
experiments (\cref{sec:sp:eval}) or restate these as design properties rather than evaluated ones.}
