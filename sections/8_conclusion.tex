...
architecture that handles a variable, changing set of candidate paths, and a \emph{feature-wise modulation}
of path features by the intent vector that lets a single policy serve the whole spectrum of objectives
without retraining. \Cref{ch:multipath} asked what happens if we use several paths at once and optimize the
...
comparison for the multipath system, and porting the non-stationary dynamics into the NS-3 backend so
packet-level results can confirm the mock findings. On the system side: replacing the TCP-like subflows with
