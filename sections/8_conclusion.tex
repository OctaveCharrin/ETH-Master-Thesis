...
reorder them. Second, that the payoff of joint rate-and-split control is \emph{not} a fixed quantity, and
that the variable governing it is not the one we expected. It is tempting to say such control matters
when the network is volatile; what we measured is that it matters when path capacity is unevenly
distributed relative to what an even split would place on each path, and that churn and bursts matter
because they destroy that evenness rather than because motion is itself the relevant property. A
stationary but asymmetric network defeats application-only adaptation as thoroughly as a churning one.
The payoff also scales with what the application asks for: the stricter its delay budget, the more a poor
allocation costs. Both findings sharpen when, and when not, to reach for the more sophisticated
mechanism.
