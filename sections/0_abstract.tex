...
Multipath QUIC beats all seven comparison points under all three application intents, including a
WebRTC-style deployed stack, by 0.065 to 0.143 QoE -- three to seven times the spread between training
...
cells to none, which makes it the largest measured effect in this thesis and inverts the usual trade,
since the dynamic-input design is also less than half the size of the fixed one it replaces. The same
architecture carries the single-path study, where it raises goodput by 41\% over a fixed-action agent
and brings a learned selector within 0.25\% of a per-context oracle while probing one path per decision
instead of thirty.
