...
%     touched the dashed lines; they now have separate lanes;
...
\begin{tikzpicture}[
  ...
  % --- feedback: per-frame delivery reward, own lane at y = -2.1 ---
  \draw[sig] (rx.south) -- (15.0,-2.1) -- node[lbl, pos=0.45, below]
    {per-frame delivery reward} (8.15,-2.1) -- (8.15,3.0) -- (tr.south east);
  % --- feedback: application reward, own lane at y = -3.0 ---
  \draw[sig] (15.0,-2.1) -- (15.0,-3.0) -- node[lbl, pos=0.5, below]
    {application reward: VMAF, latency, jitter, loss over the window its bitrate governed}
    (-2.9,-3.0) -- (-2.9,3.4) -- (app.west);
\end{tikzpicture}
\end{document}
